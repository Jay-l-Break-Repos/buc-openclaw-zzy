 ...
 *
 * If all URIs fail the app continues running with isReady() === false.
 * The chat routes fall back to an in-memory store so the API remains
 * fully functional even without a real MongoDB instance.
 *
 * connectDB() retries in the background so a late-starting sidecar will
 * eventually be picked up automatically.
 */
...
/** Returns true once Mongoose has a live connection. */
 ...
 * Errors are handled internally — the caller never needs to catch.
 */
